...
export function HomeClient({
  ...
  useEffect(() => {
    ...
    const handler = () => {
      const supabase = createClient();
      supabase.auth.getUser().then(({ data }) => {
        setBookmarkedIds(getBookmarks(data.user?.id ?? null));
      });
    };
  ...
  const goToPrevBanner = () =>
    setCurrentBannerIndex((prev) => (prev - 1 + initialBanners.length) % initialBanners.length);
  ...
  const campaignsByCategory = useMemo(() => {
    let filtered = initialCampaigns.filter(
      (c) => new Date(c.application_deadline) >= new Date()
    );
    ...
    return groups;
  }, [initialCampaigns, selectedRegion, selectedType, selectedCategory, deliveryCategoryId]);

  const visibleCategories = useMemo(
    () => initialCategories.filter((cat) => (campaignsByCategory[cat.id]?.length ?? 0) > 0),
    [initialCategories, campaignsByCategory]
  );

  const premiumCampaigns = useMemo(
    () => initialCampaigns.filter((c) => c.campaign_type === "paid").slice(0, 8),
    [initialCampaigns]
  );

  const featuredCategories = useMemo(
    () => initialCategories.filter((c) => c.is_featured),
    [initialCategories]
  );

  const bookmarkedCampaigns = useMemo(
    () => initialCampaigns.filter((c) => bookmarkedIds.includes(c.id)),
    [initialCampaigns, bookmarkedIds]
  );

  const displayCategories = showAllCategories ? initialCategories : featuredCategories;
    ...
    <div className="min-h-screen bg-gray-50/50">
                    ...
                    {initialBanners.map((_, index) => (
                      <button
                        ...
                        className={`h-2 rounded-full transition-all ${
                          index === currentBannerIndex ? "w-8 bg-primary" : "w-2 bg-white/60"
                        }`}
                      />
                    ))}
                  </div>
      ...
      <section className="border-b border-gray-100 bg-white shadow-sm">
        ...
        <div className="container mx-auto px-4 py-5">
          <div className="flex overflow-x-auto gap-4 pb-1 scrollbar-hide md:flex-wrap md:justify-center md:gap-5">
            {displayCategories.map((category, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative flex h-14 w-14 items-center justify-center rounded-full text-2xl transition-all duration-200 md:h-16 md:w-16 ${
                      isSelected
                  ...
                  >
                    {category.icon || "📦"}
                    {isSelected && (
                      <span className="absolute -bottom-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-white text-[8px] font-bold text-primary shadow">
                        ✓
                      </span>
                    )}
                  </div>
                  <span
                    className={`text-[11px] font-medium transition-colors md:text-xs ${
          ...
          {initialCategories.length > featuredCategories.length && (
